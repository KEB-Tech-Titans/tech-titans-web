...
const OverallDefectRateProgressBar: React.FC<OverallDefectRateProgressBarProps> = ({ data }) => {
  ...
  const latestData = data[data.length - 1]; // 가장 최근 데이터를 사용
  ...
  return (
    <div className="text-center mb-8">
      ...
      <ProgressBar value={latestData.defectRate} label={`Defect Rate: ${latestData.defectRate.toFixed(2)}%`} />
    </div>
  );
};
